...
const VISIBILITY_CONFIG = {
  public: { label: "Public", icon: "🌐", color: "#4fc3ff", bg: "rgba(79,195,255,0.12)", desc: "Anyone can find and view this file" },
  private: { label: "Private", icon: "👁️", color: "#a78bfa", bg: "rgba(124,106,255,0.12)", desc: "Only visible to you on Blok" },
  locked: { label: "Locked", icon: "🔒", color: "#4fffb0", bg: "rgba(79,255,176,0.12)", desc: "Encrypted — only your wallet can decrypt" },
};

export function Upload({ onSuccess }: UploadProps) {
  const account = useCurrentAccount();
  const { mutateAsync: signMessage } = useSignPersonalMessage();
  const { anchorBlob } = useAnchor();
  const [items, setItems] = useState<FileItem[]>([]);
  const [dragOver, setDragOver] = useState(false);
  const [defaultVisibility, setDefaultVisibility] = useState<"public" | "private" | "locked">("private");
  ...
  function addFiles(files: File[]) {
    const newItems: FileItem[] = files.map(f => ({
      ...
      blobId: "", error: "", visibility: defaultVisibility,
    }));
  ...
  async function uploadAll() {
    ...
    const hasLocked = waiting.some(i => i.visibility === "locked");
    if (hasLocked) {
    ...
    for (const item of waiting) {
      try {
        ...
        if (item.visibility === "locked" && key) {
          update(item.id, { status: "encrypting", progress: 25 });
          data = await encryptFile(item.file, key);
        } else {
          update(item.id, { status: "encrypting", progress: 25 });
        ...
        stored.unshift({
          ...
          isLocked: item.visibility === "locked",
          isEncrypted: item.visibility === "locked",
    ...
    <div style={{ animation: "fadeUp 0.35s cubic-bezier(0.16,1,0.3,1) forwards" }}>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        <p style={{ fontSize: "13px", color: "#8888aa" }}>
          Choose how each file is stored — public, private, or locked with encryption.
        </p>
      </div>
      ...
      {cameraError && (
        <div style={{
          ...
          fontSize: "13px", color: "#ff4f6a", display: "flex", alignItems: "center", gap: "8px",
        }}>
      ...
      {mode === "camera" && cameraReady && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: "6px" }}>
              {["photo", "video"].map(m => (
                <button key={m} onClick={() => setCameraMode(m as "photo" | "video")} style={{
                  ...
                  color: cameraMode === m ? "#a78bfa" : "#8888aa", transition: "all 0.15s",
                }}>
                  {m === "photo" ? "📷 Photo" : "🎬 Video"}
                </button>
              ))}
            ...
            <button onClick={switchToFile} style={{
              background: "rgba(255,79,106,0.08)", border: "1px solid rgba(255,79,106,0.15)",
          ...
          <div style={{ position: "relative", aspectRatio: "16/9", background: "#000" }}>
            ...
            {recording && (
              <div style={{
                ...
                <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "white", animation: "pulse 1s ease infinite" }} />
                REC
              </div>
            )}
          </div>
          <div style={{ padding: "14px 16px", display: "flex", gap: "10px", justifyContent: "center" }}>
            {cameraMode === "photo" ? (
              ...
              <button onClick={recording ? stopRecording : startRecording} style={{
                padding: "10px 28px", borderRadius: "10px", border: "none",
                background: recording ? "linear-gradient(135deg, #ff4f6a, #c0392b)" : "linear-gradient(135deg, #7c6aff, #6355e0)",
                color: "white", fontWeight: "600", fontSize: "14px", cursor: "pointer",
                boxShadow: recording ? "0 0 20px rgba(255,79,106,0.4)" : "0 0 20px rgba(124,106,255,0.35)",
              }}>
      ...
      {mode === "file" && (
        <div>
          ...
          <div style={{
            display: "flex", gap: "8px", marginBottom: "1rem", flexWrap: "wrap",
          }}>
            <span style={{ fontSize: "12px", color: "#8888aa", alignSelf: "center", marginRight: "4px" }}>
              New files default:
            </span>
            {(["public", "private", "locked"] as const).map(v => {
              const cfg = VISIBILITY_CONFIG[v];
              const active = defaultVisibility === v;
              return (
                <button key={v} onClick={() => setDefaultVisibility(v)} style={{
                  display: "flex", alignItems: "center", gap: "6px",
                  padding: "6px 14px", borderRadius: "100px", border: "none",
                  cursor: "pointer", fontSize: "12px", fontWeight: "600",
                  background: active ? cfg.bg : "rgba(255,255,255,0.04)",
                  color: active ? cfg.color : "#8888aa",
                  outline: active ? `1.5px solid ${cfg.color}` : "1.5px solid transparent",
            ...
            <span style={{ fontSize: "12px", color: "#55556a", alignSelf: "center", marginLeft: "4px" }}>
              — {VISIBILITY_CONFIG[defaultVisibility].desc}
            </span>
          </div>
          ...
          >
            ...
            {items.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {items.map(item => {
                  const cfg = VISIBILITY_CONFIG[item.visibility];
                  return (
                    <div key={item.id} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            {item.status === "waiting" && (
                              <div style={{ display: "flex", gap: "4px" }}>
                                {(["public", "private", "locked"] as const).map(v => {
                                  ...
                                  return (
                                    <button key={v} onClick={() => update(item.id, { visibility: v })} style={{
                                      padding: "3px 8px", borderRadius: "100px", border: "none",
                                      cursor: "pointer", fontSize: "10px", fontWeight: "600",
                                      background: active ? c.bg : "rgba(255,255,255,0.05)",
                                      color: active ? c.color : "#55556a",
                                      outline: active ? `1px solid ${c.color}` : "1px solid transparent",
                                      transition: "all 0.15s",
                                    }}>
                                      {c.icon}
                                    </button>
                            ...
                            {(item.status === "encrypting" || item.status === "uploading") && (
                              <span style={{
                                ...
                                {item.status === "encrypting"
                                  ? item.visibility === "locked" ? "🔒 Encrypting..." : "⚙️ Processing..."
                                  : "🌊 Uploading..."}
                              </span>
                            )}
                          </div>
                        ...
                        {item.status === "done" && item.blobId && (
                          <div style={{ fontSize: "10px", color: "#55556a", fontFamily: "monospace", marginTop: "4px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {item.blobId}
                          </div>
                        )}

                        {item.status === "error" && (
                          <div style={{ fontSize: "11px", color: "#ff4f6a", marginTop: "3px" }}>{item.error}</div>
                        )}
                      </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
            {items.length > 0 && (
              <button onClick={() => fileInput.current?.click()} style={{
                ...
                color: "#8888aa", cursor: "pointer", fontWeight: "500", transition: "all 0.15s",
              }}>
                + Add more
              </button>
            )}
            ...
            {hasWaiting && (
              <button onClick={uploadAll} disabled={uploading} style={{
                ...
                background: uploading ? "rgba(124,106,255,0.4)" : "linear-gradient(135deg, #7c6aff, #6355e0)",
                color: "white",
                ...
                {uploading
                  ...
                  : <>⬆ Upload {items.filter(i => i.status === "waiting").length} file{items.filter(i => i.status === "waiting").length !== 1 ? "s" : ""}</>
                }
              </button>
            )}
